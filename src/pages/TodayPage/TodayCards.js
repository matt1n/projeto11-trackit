...
export default function TodayCards({info, showToday}) {
    ...
    return(
        <TodayCard grayText={grayText} correctGreen={correctGreen} info={info}>
                <CardTitleChecked grayText={grayText} correctGreen={correctGreen}>{info.name}</CardTitleChecked>
                <span>
                    <TextNormal grayText={grayText}>Sequência atual: </TextNormal>
                    <TextCurrent grayText={grayText} correctGreen={correctGreen} done={info.done}>{info.currentSequence} dias</TextCurrent>
                </span>
                <span>
                    <TextNormal grayText={grayText}>Seu recorde:</TextNormal>
                    <TextHighest grayText={grayText} correctGreen={correctGreen} current={info.currentSequence} highest={info.highestSequence}>{info.highestSequence} dias</TextHighest>
                </span>
                <ion-icon onClick={()=>checkCard()} name="checkbox"></ion-icon>
            </TodayCard>
    )
};
...
const TodayCard = styled.div`
    ...
    ion-icon{
        ...
        top: 5px;
        font-size: 80px;
        color: ${props=> props.info.done ? props.correctGreen : props.grayText};
    }
`
...
const TextHighest = styled.p`
    ...
    font-size: 13px;
    margin-right: 3px;
    color: ${props=> props.current===props.highest && props.highest!==0 ? props.correctGreen : props.grayText};
`
